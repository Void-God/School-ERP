...
while($row = mysqli_fetch_assoc($query_run)){
  if($row['user_ID'] == $search || strpos(strtolower($row['user_Name']),strtolower($search)) !== false)
  {
    echo '<div style="border : 1px solid black;padding:10px">';
        echo "<form>";
          echo '<input type="text" style="display:none" id="data_on_set_eos_'.$flag.'" value='.$row['user_ID'].'></input>';
          ...
          echo '<select id="class_to_assign_eos_'.$flag.'" >';
          while ($rows = mysqli_fetch_assoc($query_for))
          {
            echo '<option value="'.$rows['class_section'].'">'.$rows['class_section'].'</option>';
          }
          echo '</select>';
          echo '<button class="btn btn-danger" type="button" onclick="changeclass_eos('.$flag.')">CHANGE</button>';
